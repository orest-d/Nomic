...
package eu.lateral.nomic.meno.evaluatortrans
...
object Util extends CommonUtils{
  type Translator = EvaluatorTrans
...
class IdentifierT(val obj: Identifier) extends AnyVal {
  ...
  def binaryOperatorObjectNameE = binaryOperatorObjectName.evalname
  
  def binaryTranslationClass = s"""|class $binaryOperatorObjectNameE(val obj:$binaryOperatorObjectName) extends AnyVal{
    |  import Util._
    |  def left(implicit evaluator:Evaluator)  = obj.left.eval
    |  def right(implicit evaluator:Evaluator) = obj.right.eval
    |
    |  def eval(implicit evaluator:Evaluator) = StringResult(s"$$left $value $$right")
    |}
    |""".stripMargin
  def binaryCase = "    case x:%-20s => x.eval\n".format(binaryOperatorObjectName)
  def binaryImplicit = s"  implicit def to$binaryOperatorObjectNameE(obj:$binaryOperatorObjectName):$binaryOperatorObjectNameE = new $binaryOperatorObjectNameE(obj)\n"
  def translate(implicit translator: Translator) = value
}
...
class TokenStatementT(val obj: TokenStatement) extends AnyVal {
  ...
  def evalname(implicit translator: Translator) = name.evalname
  def implicits(implicit translator: Translator) = s"  implicit def to$evalname(obj:$astname):$evalname = new $evalname(obj)\n"
  def classes(implicit translator: Translator) = s"""|class $evalname(val obj:$astname) extends AnyVal{
    |  import Util._
    |  def value = obj.value
    |
    |  def eval(implicit evaluator:Evaluator) = StringResult(value)
    |}
    |""".stripMargin

  def cases(implicit translator: Translator) = "    case x:%-20s => x.eval\n".format(astname)
...
class RuleStatementT(val obj: RuleStatement) extends AnyVal {
  ...
  def classes(implicit translator: Translator) = s"""|class $evalname(val obj:$astname) extends AnyVal{
    |  import Util._
    |$methods
    |  def eval(implicit evaluator:Evaluator) = StringResult(s"$sequence")
    |}
    |""".stripMargin
...
class PatternRuleElementT(val obj: PatternRuleElement) extends AnyVal {
  ...
  def methodDefinition(implicit translator: Translator) = s"  def $refname(implicit evaluator:Evaluator) = $body\n"
  ...
  def bodySimple(implicit translator: Translator) = s"obj.$refname.eval"
  def bodyBoolean(implicit translator: Translator) = s"""BooleanResult(obj.$refname)"""
  def bodyOption(implicit translator: Translator) = s"obj.$refname.map(_.eval).mkString"
  def bodyList(implicit translator: Translator) = s"ListResult(obj.$refname.list.map(_.eval))"
...
class BinaryStatementT(val obj: BinaryStatement) extends AnyVal {
  ...
  def classes(implicit translator: Translator) = s"""$derivedClasses
    |class $evalname(val obj:$astname) extends AnyVal{
    |  import Util._
    |
    |  def eval(implicit evaluator:Evaluator) = evaluator(obj.content)
    |}
    |""".stripMargin
...
class MainT(val obj: Main) extends AnyVal {
  ...
  def translate(implicit translator: Translator) = s"""package $pkg.evaluator
    ...
    |object Util{
    |$implicits
    |  def notSupported(obj:ASTObject) = ErrorResult("Evaluation of $${obj.getClass().getName()} not supported.") 
    |}
    |
    |$classes
    |
    ...
    |case class ListResult(list:List[Result])  extends Result
    |
    |class Evaluator{
    |  import Util._
    |  implicit def evaluator:Evaluator = this
    |  def apply(obj:ASTObject):Result = obj match{
    |$cases
    |    case _ => notSupported(obj)
    |  }
    |}
    |
    |object Evaluator{
    |  def formatResult(result:Result)=result match{
    |    case ErrorResult(message) => s"ERROR:   $$message"
    |    case BooleanResult(value) => if (value) "TRUE" else "FALSE"
    |    case StringResult(value)  => s"STRING:  $$value"
    |    case ListResult(list)     => s"LIST:    "+list.mkString(", ")
    |    case _                    => s"UNKNOWN: $$result"
    |  }
    |
    |  def main(arg:Array[String]){
    |    val path=arg(0)
    |    val ast = Parser.fromFile(path).asInstanceOf[Main]
    |    val evaluator = new Evaluator
    |    println(formatResult(evaluator(ast)))
    |  }
    |}
    |""".stripMargin
}

class EvaluatorTrans extends MenoTranslatorBase {
  import Util._

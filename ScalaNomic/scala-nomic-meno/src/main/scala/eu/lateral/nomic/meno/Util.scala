...
import eu.lateral.nomic.ObjectTranslators.ASTObjectOT
...
trait GroupUtil[T] extends Util {
  self: ObjectTranslators.ASTObjectOT[T, GroupStatement] =>
  def referencedGroupMembers(name: Identifier, translator: Translator):List[Identifier] = {
    val option = findStatement(name, name.value)
    if (option.isDefined){
      val statement = option.get
      if (statement.groupStatement.isDefined)
        groupMembers(statement.groupStatement.get,translator)
      else
        List(name)
    }
    else {
      translator.error(PositionError(s"Reference $name not found", name.pos))
      List()
    }
  }
  
  def groupMembers(group: GroupStatement, translator: Translator) = {
    val m = for (
        name <- group.sequence.list;
        member <- referencedGroupMembers(name, translator)) yield member
    scala.collection.mutable.LinkedHashSet(m:_*).toList
  }
  def groupMembersSequence = new ObjectTranslators.ListOT(self / {(t:Translator,x:GroupStatement) => groupMembers(x,t)})
}
